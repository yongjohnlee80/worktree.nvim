...
--
-- LEGACY FALLBACK (per ADR 0007 Phase 1 Step 1.2 + ADR 0006
-- §"worktree.nvim migration plan"). The dispatcher in `git.lua` now
-- prefers `auto-core.git.worktree` + `auto-core.git.repo` when
-- present; this file is what the dispatcher falls back to when
-- auto-core isn't installed. Retires after one minor release.
-- DO NOT add new functions here — extend auto-core instead.
